...
static bool gpuinfo_qcom_npu_init(void) { return true; }
...
static const char *gpuinfo_qcom_npu_last_error_string(void) { return "QCOM-NPU error"; }
...
static void find_nsp_thermal_zones(struct gpu_info_qcom_npu *npu) {
  ...
  while ((ent = readdir(dir)) != NULL) {
    ...
    snprintf(npu->nsp_thermal_paths[npu->nsp_thermal_count], sizeof(npu->nsp_thermal_paths[0]),
...
static void gpuinfo_qcom_npu_populate_static_info(struct gpu_info *_gpu_info) {
  ...
  if (arch_ver) {
    snprintf(static_info->device_name, sizeof(static_info->device_name), "Qualcomm Hexagon v%x NPU", arch_ver);
...
static void gpuinfo_qcom_npu_get_running_processes(struct gpu_info *_gpu_info) { (void)_gpu_info; }
