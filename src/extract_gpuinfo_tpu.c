...
bool refresh_tpu_cache(void) {
  ...
  for (int i = 0; i < tpu_chip_count; i++) latest_chips_usage_data[i].pid = _pids[i];
...
bool gpuinfo_tpu_init(void) {
  ...
  error_msg = dlerror();
  if (error_msg != NULL) goto dlsymb_cleanup;
  ...
  dlsymb_cleanup:
      fprintf(stderr, "TPU support error, libtpuinfo %s symbol cannot be resolved with error: %s\n", symbol_name, error_msg);
      return false;
}
