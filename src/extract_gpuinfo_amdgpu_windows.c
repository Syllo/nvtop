/*
 * AMD GPU Information Extraction for Windows
 * Copyright (C) 2025
 *
...
/*
 * IMPLEMENTATION STATUS: Fully Functional (using DXGI/PDH)
 *
 * This implementation provides AMD GPU monitoring on Windows using:
 * - DXGI (DirectX Graphics Infrastructure) for GPU enumeration
 * - PDH (Performance Data Helper) API for performance metrics
 *
 * This is a lightweight implementation that doesn't require the ADL SDK.
 * For advanced features (detailed clock speeds, voltage, advanced power management),
 * the ADL SDK can be integrated in the future.
 *
 * Supported Metrics:
 ...
 *
 * Note: Temperature, fan speed, and detailed power metrics require ADL SDK.
 */
...
// AMD GPU device handle
struct amd_gpu_handle {
  UINT adapter_index;
...
static struct amd_gpu_handle *gpu_handles = NULL;
static unsigned num_gpus = 0;

// Error handling
static char error_string[1024] = "";

static const char *gpuinfo_amdgpu_last_error_string(void) { return error_string[0] ? error_string : "No error"; }
...
// Get AMD device handles
static bool gpuinfo_amdgpu_get_device_handles(struct list_head *devices, unsigned *count) {
  IDXGIFactory *pFactory = NULL;
  IDXGIAdapter *pAdapter = NULL;
  HRESULT hr;
  UINT adapter_index = 0;
  unsigned amd_gpu_count = 0;

  *count = 0;

  if (!pdh_initialized) {
    snprintf(error_string, sizeof(error_string), "AMD GPU monitoring not initialized");
    return false;
  }
  ...
  while (pFactory->lpVtbl->EnumAdapters(pFactory, adapter_index, &pAdapter) != DXGI_ERROR_NOT_FOUND) {
    ...
    if (SUCCEEDED(hr) && desc.VendorId == 0x1002) { // AMD VendorId
      amd_gpu_count++;
    }
  ...
  if (amd_gpu_count == 0) {
    ...
    return false; // No AMD GPUs found
  }

  // Allocate handles
  gpu_handles = calloc(amd_gpu_count, sizeof(struct amd_gpu_handle));
  if (!gpu_handles) {
  ...
  while (pFactory->lpVtbl->EnumAdapters(pFactory, adapter_index, &pAdapter) != DXGI_ERROR_NOT_FOUND) {
    ...
    if (SUCCEEDED(hr) && desc.VendorId == 0x1002) {
      ...
      gpu_info->vendor = &gpu_vendor_amdgpu;

      // Store handle info
      struct amd_gpu_handle *handle = &gpu_handles[gpu_idx];
      ...
      snprintf(gpu_info->pdev, PDEV_LEN, "amd%u", adapter_index);
...
static void gpuinfo_amdgpu_populate_static_info(struct gpu_info *_gpu_info) {
  ...
  sscanf(_gpu_info->pdev, "amd%u", &adapter_idx);

  if (adapter_idx >= num_gpus)
    return;

  struct amd_gpu_handle *handle = &gpu_handles[adapter_idx];
  ...
  // AMD discrete GPUs typically have dedicated VRAM
  _gpu_info->static_info.integrated_graphics =
      (desc->DedicatedVideoMemory < (512 * 1024 * 1024)); // < 512MB likely integrated
}
...
static void gpuinfo_amdgpu_refresh_dynamic_info(struct gpu_info *_gpu_info) {
  ...
  if (!handle->gpu_util_counter) {
    ...
    // Try to add power counter (may not be available on all AMD GPUs without ADL)
  ...
  // Sleep briefly to allow counters to stabilize
  Sleep(100);
  status = PdhCollectQueryData(pdhQuery);
  if (status != ERROR_SUCCESS) {
    return;
...
static void gpuinfo_amdgpu_get_running_processes(struct gpu_info *_gpu_info) {
  ...
  // This uses the same technique as Intel GPU monitoring
        ...
        if (status == ERROR_SUCCESS && counterValue.CStatus == PDH_CSTATUS_VALID_DATA) {
          ...
          for (unsigned int i = 0; i < process_count; i++) {
            if (process_list[i].pid == pid) {
              process_list[i].utilization += util;
              found = true;
